...
allocate( A(1:d,1:d) ) ;   call ginibre(optg, d, d, A) ;   call gram_schmidt_modified(d, d, A, ru) ;   deallocate( A )
...
call ginibre(optg, d, d, Z) ;   call lapack_zgeqrfp(d, Z, Q, R) ;   ru = Q
deallocate( Z, Q, R )

end
!------------------------------------------------------------------------------------------------------------------------------------
subroutine ru_hurwitz(optg, d, ru) ! Returns a dxd random unitary matrix from the Circular Unitary Ensemble (CUE) using Hurwitz's parametrization
...
do i = 1, d-1   ! Loop for the product of Ei: E_1*E_2*...*E_(d-1)
  ...
  do j = i, 1, -1  ! Loop for the products among the Ejk matrices, needed for obtaining Ei      
    Ejk = id
    call rng(optg, 1, rn) ;   xi = rn(1) ;   phi = dasin(xi**(1.d0/(2.d0*dble(j))))
    call rng(optg, 1, rn) ;   psi = 2.d0*pi*rn(1) 
    Ejk(j,j) = dcos(phi)*(dcos(psi) + (0.d0,1.d0)*dsin(psi)) ;   Ejk(k,k) = conjg(Ejk(j,j))
    if (j == 1) then
      call rng(optg, 1, rn) ;   chi = 2.d0*pi*rn(1)
...
call rng(optg, 1, rn) ;   alpha = 2.d0*pi*rn(1)
do j = 1, d ;   do k = 1, d 
...
!###################################################################################################################################
subroutine rug(optg, d, ru)  ! Calls the choosed random unitary matrix generator
implicit none
integer :: d  ! Dimension of the Random Unitary Matrix
complex(8) :: ru(1:d,1:d)  ! Random Unitary Matrix
character(10), dimension(5) :: optg  ! Options for the generators

     if ( (optg(3) == "std") .or. (optg(3) == "gso") ) then ;   call ru_gram_schmidt(optg, d, ru)
else if ( optg(3) == "hhr" ) then ;   call ru_householder(optg, d, ru)
else if ( optg(3) == "hurwitz" ) then ;   call ru_hurwitz(optg, d, ru)
     endif
